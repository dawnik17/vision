...
class CosineSimilarityLoss(nn.Module):
    ...
    def forward(self, image_embeddings, text_embeddings):
        ...
        score = torch.max(fill_tensor, positive_score - margin_tensor)

        positive_class_weight = ((batch ** 2) - batch) / batch
        filler = torch.ones(batch, batch, device=image_embeddings.device)
        filler.diagonal().fill_(positive_class_weight)

        score = filler * score

        return score.sum() if self.reduction == "sum" else score.mean()
...
class HingeLoss(nn.Module):
    ...
    def forward(self, image_embeddings, text_embeddings):
        ...
        score = torch.max(fill_tensor, score)
...
class BCELoss(nn.Module):
    def __init__(self, reduction, temperature, positive_class_weight=1):
        super(BCELoss, self).__init__()
        self.temperature = temperature
        self.positive_class_weight = positive_class_weight
        self.reduction = reduction
        nn.criterion = nn.BCELoss(reduction="none")
    ...
    def forward(self, image_embeddings, text_embeddings):
        ...
        target = torch.eye(batch, device=image_embeddings.device)
        score = nn.criterion(score, target)
...
class PairwiseLoss(nn.Module):
    ...
    def forward(self, image_embeddings, text_embeddings):
        ...
        image_pair = self.sigmoid(base - images_similarity)
        text_pair = self.sigmoid(base - texts_similarity)

        # Remove diagonal elements and create a new tensor
        image_pair = image_pair[~torch.eye(batch, dtype=bool)]
        text_pair = text_pair[~torch.eye(batch, dtype=bool)]
        labels = torch.ones(batch * (batch-1), device=self.device).float()
        
        total_loss = self.criterion(image_pair, labels) + self.criterion(text_pair, labels)
        return total_loss / 2
